...
import { Mail, Lock, ShieldCheck, Sun, Moon, ArrowRight } from 'lucide-react';
...
/**
 * Super Minimal Centered Login Page.
 * Displays only the login inputs with clean placeholders ("Email", "Password").
 */
const Login = () => {
      ...
      <div className="max-w-md w-full bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800 p-8 rounded-2xl shadow-sm flex flex-col gap-6 animate-fade-in">
        ...
        <form onSubmit={handleFormSubmit} className="space-y-4" autoComplete="off">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-3 rounded-xl border border-zinc-200 dark:border-zinc-805 dark:border-zinc-800 bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 focus:outline-none focus:ring-1 focus:ring-zinc-950 dark:focus:ring-zinc-50 text-xs transition-all shadow-sm"
              placeholder="Email"
              required
              autoComplete="off"
            />
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 bg-zinc-950 dark:bg-zinc-100 hover:bg-zinc-850 dark:hover:bg-zinc-200 text-white dark:text-zinc-955 dark:text-zinc-950 font-bold text-xs rounded-xl shadow-sm transition-all disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer mt-4"
          >
            {loading ? (
              <>
        ...
        <div className="border-t border-zinc-100 dark:border-zinc-800/80 pt-4 text-center">
          <p className="text-xs text-zinc-500 dark:text-zinc-400 font-medium">
            Don't have an adjuster account?{' '}
            <Link
              to="/register"
